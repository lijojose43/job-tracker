...
function addOrUpdate(job) {
  ...
  if (existing) {
    ...
    job.createdAt = job.createdAt || nowTs();
...
function buildEmailTemplate(job) {
  ...
  const lines = [
    ...
    "",
    `I hope this message finds you well. I am writing to express my interest in teaching opportunities at your institution.`,
    "",
    "I have attached my resume for your review. I would welcome the opportunity to discuss how my background can support your academic objectives.",
    "",
    "If any additional information would be helpful, please let me know. I am available at your convenience for a conversation or interview.",
    "",
...
function sendEmail(companyName, email) {
  ...
  if (opened) return;
  // Fallback to default email client via mailto
  try {
...
function openProfile() {
  ...
  if (profileForm.name) profileForm.name.value = p.name || "";
  if (profileForm.phone) profileForm.phone.value = p.phone || "";
  if (profileForm.email) profileForm.email.value = p.email || "";
...
function openEdit(job) {
  ...
  if (jobForm.contactWebsite)
    jobForm.contactWebsite.value = job.contactWebsite || "";
...
if (profileForm) {
  profileForm.addEventListener("submit", (e) => {
    ...
    try {
      ...
      const val = {
        name: (data.get("name") || "").toString().trim() || "Your Name",
        phone: (data.get("phone") || "").toString().trim() || "Your Phone",
        email: (data.get("email") || "").toString().trim() || "your@email.com",
      };
...
// Profile sheet drag handle
(function () {
  const handle = document.getElementById("profileSheetHandle");
  if (!handle || !profileForm || !profileModal) return;
  let dragging = false;
  let startY = 0;
  let currentY = 0;
  function onStart(e) {
    ...
    startY = e.touches ? e.touches[0].clientY : e.clientY;
    currentY = 0;
    profileForm.style.transitionProperty = "transform";
  }
  function onMove(e) {
    if (!dragging) return;
    const y = e.touches ? e.touches[0].clientY : e.clientY;
    currentY = Math.max(0, y - startY);
    profileForm.style.transform = `translateY(${currentY}px)`;
  }
  function onEnd() {
    if (!dragging) return;
    dragging = false;
